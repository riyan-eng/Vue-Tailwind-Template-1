<template>
    <PrivateLayout>
        <button @click="toggleModalInsertOne" class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 mx-auto transition duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-4 sm:px-8 py-2 text-xs sm:text-sm mb-2">
            Open large modal
        </button>
            ...
            <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
                <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                    ...
                    <div class="flex items-center justify-start w-full">
                        ...
                        <button class="focus:outline-none focus:ring-2 focus:ring-offset-2  focus:ring-gray-400 ml-3 bg-gray-100 transition duration-150 text-gray-600 ease-in-out hover:border-gray-400 hover:bg-gray-300 border rounded px-8 py-2 text-sm" @click="toggleModalInsertOne">Cancel</button>
                    </div>
                    <button class="cursor-pointer absolute top-0 right-0 mt-4 mr-5 text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out rounded focus:ring-2 focus:outline-none focus:ring-gray-600" @click="toggleModalInsertOne" aria-label="close modal" role="button">
        ...
        <Vue3EasyDataTable buttons-pagination show-index :theme-color="'#f48225'" v-model:items-selected="itemsSelected"
            :headers="headers" :items="items" :rows-items="[10, 25, 50, 100]" :rows-per-page="10" :loading="loading">
            <template #item-action="item">
                <div class="flex">
                    ...
                    <div class="h-7 w-7 p-1 cursor-pointer text-red-500">
                        <TrashIcon />
                    </div>
                </div>
...
export default {
    components: {
        PrivateLayout, Vue3EasyDataTable, TrashIcon, PencilSquareIcon, EyeIcon, XMarkIcon
    },
    created() {
        this.fetchTodo()
    },
