<template>
    <div v-if="showModalDeleteOne"
        class="overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center flex">
        <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
            <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                <div class="w-full flex justify-start text-gray-600 mb-3">
                    <TrashIcon class="w-14"/>
                </div>
                <h1 class="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Are you sure want to delete id: {{idItem}}?
                </h1>
                
                <div class="flex items-center justify-start w-full">
                    <button @click="onDeleteOne(idItem)"
    ...
    <div v-if="showModalDeleteOne" class="opacity-25 fixed inset-0 z-40 bg-black"></div>
</template>
...
export default {
    ...
    emits: ["emtCloseModalDeleteOne", "emtOnDeleteOne"],
