                ...
                <form @submit.prevent="onSubmit">
                    ...
                    <select v-model="item.roleId"
                        ...
                        <option v-for="(role, index) in roles" :key="index" :value="role.roleId">
                            <span>{{role.bussinesType?role.bussinesType.bussinesTypeCode + ' - ': null}}</span>
...
export default {
    ...
    props: ['showModalInsertOne', 'roles', 'permissions'],
    methods: {
        closeModalInsertOne() {
            this.$emit('emtCloseModalInsertOne')
        },
        onSubmit() {
            if (this.item.permissionId === '' && this.item.roleId === ''){
