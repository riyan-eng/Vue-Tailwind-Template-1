    ...
    <div v-if="showModalInsertOne"
        ...
        <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
            <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                ...
                <h1 class="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Insert Permission
                </h1>
                <form @submit.prevent="onSubmit">

                    <label for="name" class="text-gray-800 text-sm font-bold leading-tight tracking-normal">Owner
                        Name</label>
                    <input id="name" v-model="item.permissionName"
                        class="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border"
                        placeholder="James" />
                    <label for="name" class="text-gray-800 text-sm font-bold leading-tight tracking-normal">Owner
                        Name</label>
                    <input id="name" v-model="item.permissionPath"
                        class="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border"
                        placeholder="James" />
...
export default {
    ...
    props: ['showModalInsertOne'],
    methods: {
        closeModalInsertOne() {
            this.$emit('emtCloseModalInsertOne')
        },
        onSubmit() {
            if (this.item.permissionName === '' && this.item.permissionPath === '') {
                alert('tidak boleh kosing')
                return
            }
            this.$emit('emtOnSubmitInsertOne', this.item)
            this.item.permissionName = ''
            this.item.permissionPath = ''
