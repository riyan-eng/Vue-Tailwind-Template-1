            ...
            <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                ...
                <form @submit.prevent="onSubmit">
                    ...
                    <select v-model="item.accessId"
                        ...
                        <option v-for="(acces, index) in access" :key="index" :value="acces.accessId">
                            <span>{{acces.accessName}}</span>
                        </option>
                    </select>
                    <label for="name" class="text-gray-800 text-sm font-bold leading-tight tracking-normal">
                        Permission</label>
                    <select v-model="item.permissionId"
                        class="mb-5 mt-2 bg-white text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border">
                        <option value="" selected>Choose a permission</option>
                        <option v-for="(permission, index) in permissions" :key="index" :value="permission.permissionsId">
...
export default {
    ...
    props: ['showModalInsertOne', 'access', 'permissions'],
    methods: {
        closeModalInsertOne() {
            this.$emit('emtCloseModalInsertOne')
        },
        onSubmit() {
            if (this.item.permissionId === '' && this.item.accessId === ''){
